...
class BotLogic extends EventEmitter {
    ...
    log(level, message) {
        ...
        else if (level === 'WAIT') icon = '⌛';
        else if (level === 'DEBUG') icon = '🐞'; // Added for debugging
    ...
    async start() {
        ...
        for (const channelId of channelIds) {
            // Assign tokens round-robin to channels
            const token = discordTokens[tokenIndex % discordTokens.length];
            const botInfo = botInfos[token];
            if (!botInfo) {
                 this.log('ERROR', `Skipping channel ${channelId} - assigned token is invalid.`);
                 continue; // Skip if the assigned token failed validation
            }
            tokenIndex++;
    ...
    getDiscordTokens() { return discordTokens; }
